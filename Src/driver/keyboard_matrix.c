...
void kbm_scan(uint8_t *scancodes){
    ...
    // Read GPIOA value
    HAL_Delay(5);
    scancodes[0] = (KBM_GPIO -> IDR >> 6) & 0x7F;
   ...
   // Clear pins
    ...
    // Read GPIOA value
    HAL_Delay(5);
    scancodes[1] = (KBM_GPIO -> IDR >> 6) & 0x7F;
    ...
    // Read GPIOA value
    HAL_Delay(5);
    scancodes[2] = (KBM_GPIO -> IDR >> 6) & 0x7F;
    ...
    // Read GPIOA value
    HAL_Delay(5);
    scancodes[3] = (KBM_GPIO -> IDR >> 6) & 0x7F;
...
uint8_t kbm_getValue(uint8_t blocking_mode){
    ...
    }while((blocking_mode == 1) && (validscancode == 0xFF));
...
char kbm_getAsciiValue(uint8_t blocking_mode){
    uint8_t key = kbm_getValue(blocking_mode);
    switch (key){
